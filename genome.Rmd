...
Note on 25 October 2019: This page is still a work-in-progress, but will be completed before 30 October 2019.
</div>
...
There exist various methods for mapping reads or loci against a reference genome. One popular method is the Burrows-Wheeler Aligner (`bwa`). You can download `bwa` from [here](https://sourceforge.net/projects/bio-bwa/files/), and installation instructions are available in various places online. We'll also use the program `samtools` to modify our output files; you can find installation instructions for this program [here](http://www.htslib.org/download/). **For the sake of time, I'm going to demonstrate how to align loci to the genome today, and you can just watch on my screen.** However, I've included all of the data necessary to do this yourself in the Github repository, so feel free to experiment on your own time.

### Prepare reference genome
Our first task is to "index" the reference genome. In brief, this is a process in which your computer organizing the reference genome in a fashion that optimizes the computational efficiency of the mapping. Assuming that we've placed our reference genome in a directory called `\reference` within our current working directoy, we can index by typing using the following command (**note**: again, this takes some time, so don't do this in class today**):
...
```{r}
Brid_R1R2_pos <- matrix(nrow=length(Brid_R1R2_BAM),ncol = 2)
for(i in 1:length(Brid_R1R2_BAM)){
	temp <- strsplit(Brid_R1R2_BAM[i],"\t")[[1]]
	Brid_R1R2_pos[i,] <- c(temp[c(1,3)])
}
```
...
```{r}
Brid_R1R2_posMatching <- Brid_R1R2_pos[Brid_R1R2_pos[,2]!="*",]
```

We can now check what proportion of our dataset was removed.
```{r}
1-(dim(Brid_R1R2_posMatching)[1]/dim(Brid_R1R2_pos)[1])
```
...
```{r}
Brid_R1R2_posMatching_reduced <- Brid_R1R2_posMatching[!duplicated(Brid_R1R2_posMatching[,1]),]
```

### Plot mapped loci.

One simple question we might have is the distribution of our loci across the genome. Let's plot the number of loci that map to each of our contigs/scaffolds.

```{r}
barplot(table(as.character(Brid_R1R2_posMatching_reduced[,2])),
        col = "orange", xlab = "", ylab = "",
        main = "3RAD loci assembled against genome", xaxt = 'n', space = 0)
axis (side = 1, labels = c("",""),
      las = 2, at = 0.5+c(0,c(length(unique(Brid_R1R2_posMatching_reduced[,2][grep("NC_", Brid_R1R2_posMatching_reduced[,2])]))-1)),
      cex.axis = 0.5, tick = TRUE)
axis (side = 1, labels = c("chrom."),
      las = 1, at = mean(c(0,c(length(unique(Brid_R1R2_posMatching_reduced[,2][grep("NC_", Brid_R1R2_posMatching_reduced[,2])]))-1))),
...
```{r}
barplot(table(as.character(Brid_R1R2_posMatching_reduced[,2][grep("NC_", Brid_R1R2_posMatching_reduced[,2])])),
        col = "orange", xlab = "", ylab = "",
        main = "3RAD loci assembled against genome", xaxt = 'n', space = 0)
axis (side = 1, labels = unique(Brid_R1R2_posMatching_reduced[,2][grep("NC_", Brid_R1R2_posMatching_reduced[,2])]),
      las = 2, at = 0.5+c(0:c(length(unique(Brid_R1R2_posMatching_reduced[,2][grep("NC_", Brid_R1R2_posMatching_reduced[,2])]))-1)),
      cex.axis = 0.5)
...
* `-t` : the maximum allowable time for our job; here, we've requested two hours
